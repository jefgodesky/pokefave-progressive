...
const init = (nodes: Element[], localStorage: Storage = window.localStorage): void => {
  const faves = localStorage.getItem('faves') ?? ''
  const faveIDs = faves.split(',').map(fave => fave.trim())
  for (const node of nodes) {
    const img = Array.from(node.children).filter(tag => tag.tagName === 'IMG')[0]
    const btn = create({ tag: 'button' })
    if (faveIDs.includes(node.getAttribute('data-pokedex') ?? '')) addClass(btn, 'is-fave')
    btn.innerHTML = star
